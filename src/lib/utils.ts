...
export function getSubtaskProgress(subtasks: { is_done: boolean }[] | undefined | null): {
  total: number;
  completed: number;
  percent: number;
} {
  const total = subtasks?.length ?? 0;
  const completed = subtasks?.filter((subtask) => subtask.is_done).length ?? 0;
  return { total, completed, percent: safePercentage(completed, total) };
}
