...
export function TourProvider({ children }: TourProviderProps) {
  ...
  const [state, setState] = useState<TourState>(() => {
    if (typeof window === "undefined") {
      return { isRunning: false, currentStepIndex: 0, completedTours: new Set<string>() }
    }
...
function TourOverlay() {
  ...
  const step = currentTour && state.currentStepIndex < currentTour.steps.length ? currentTour.steps[state.currentStepIndex] : null

  // Position the highlight box by writing directly to the DOM node rather
  // than through React state, since this is measuring/syncing with the
  // actual rendered layout of an external target element.
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div
        ref={highlightRef}
        className="fixed z-40 hidden rounded-2xl border-2 border-primary shadow-[0_0_0_9999px_rgba(0,0,0,0.6)] animate-pulse"
      />
...
const predefinedToursConst = [
  ...
  },
  {
    ...
    steps: [
      { id: "kanban-views", title: "Priority Swimlanes", content: "Toggle 'Group by priority' to split each column into high, medium, and low priority lanes.", targetSelector: "[data-tour='kanban-views']", position: "bottom" as const },
