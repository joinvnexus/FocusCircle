...
export function NotificationCenter({ initialNotifications, onOpenChange }: NotificationCenterProps) {
  // Instead of copying `initialNotifications` into local state (which would
  // need an effect to stay in sync whenever the caller streams in fresh data,
  // e.g. a realtime subscription), we track only the optimistic overrides the
  // user has triggered and derive the displayed list from props + overrides.
  const [readOverrideIds, setReadOverrideIds] = useState<Set<string>>(new Set());
  const [deletedIds, setDeletedIds] = useState<Set<string>>(new Set());

  const notifications = useMemo(() => {
    return initialNotifications
      .filter((notif) => !deletedIds.has(notif.id))
      .map((notif) => (readOverrideIds.has(notif.id) ? { ...notif, is_read: true } : notif));
  }, [initialNotifications, readOverrideIds, deletedIds]);
  ...
  const handleMarkAllRead = async () => {
    ...
    startTransition(async () => {
      ...
      if (result.error) {
        ...
        setReadOverrideIds((prev) => new Set([...prev, ...unreadIds]));
  ...
  const handleMarkSelectedRead = async () => {
    ...
    startTransition(async () => {
      ...
      if (result.error) {
        ...
        setReadOverrideIds((prev) => new Set([...prev, ...selectedArray]));
  ...
  const handleDeleteSelected = async () => {
    ...
    startTransition(async () => {
      ...
      if (result.error) {
        ...
        setDeletedIds((prev) => new Set([...prev, ...selectedArray]));
        setSelectedIds(new Set());
  ...
  return (
    <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative" aria-label="Notifications" data-tour="notification-button">
      ...
      <PopoverContent align="end" className="w-[480px] p-0 max-h-[80vh] overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" onClick={handleMarkAllRead} disabled={unreadCount === 0} className="text-xs h-8" data-tour="notif-actions">
              Mark all read
            </Button>
          </div>
        </div>

        <div className="border-b px-4 py-3" data-tour="notif-filters">
          <div className="flex items-center gap-2">
