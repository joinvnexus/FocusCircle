...
async function getWeeklyCompletedTasks(userId: string) {
  ...
  const tasksByDay = new Map<string, { id: string; title: string }[]>(labelOrder.map((label) => [label, []]));

  for (const task of data ?? []) {
    const label = new Intl.DateTimeFormat("en-US", { weekday: "short" }).format(new Date(task.updated_at));
    counts.set(label, (counts.get(label) ?? 0) + 1);
    tasksByDay.get(label)?.push({ id: task.id, title: task.title });
  }

  return labelOrder.map((day) => ({ day, completed: counts.get(day) ?? 0, tasks: tasksByDay.get(day) ?? [] }));
}
