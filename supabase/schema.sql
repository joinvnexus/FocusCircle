...
-- Subtasks table (checklist items nested under a task, used for progress roll-up)
...
-- Subtasks (access mirrors the parent task's own RLS rules above)
CREATE POLICY "Users can view subtasks of visible tasks" ON public.subtasks FOR SELECT USING (
  EXISTS (
...
CREATE POLICY "Users can create subtasks on visible tasks" ON public.subtasks FOR INSERT WITH CHECK (
  created_by = auth.uid()
  AND EXISTS (
    SELECT 1 FROM public.tasks t
    WHERE t.id = subtasks.task_id
...
CREATE POLICY "Users can update subtasks of visible tasks" ON public.subtasks FOR UPDATE USING (
  EXISTS (
    SELECT 1 FROM public.tasks t
    WHERE t.id = subtasks.task_id
      AND (t.created_by = auth.uid() OR t.assigned_to = auth.uid() OR public.is_circle_member(t.circle_id))
...
CREATE POLICY "Users can delete subtasks of visible tasks" ON public.subtasks FOR DELETE USING (
  EXISTS (
    SELECT 1 FROM public.tasks t
    WHERE t.id = subtasks.task_id
